...
import JobTable from '@/app/(admin)/admin/tuyen-dung/JobTable'
import NewsTable from '@/app/(admin)/admin/tin-tuc/NewsTable'
...
const PAGE_CONFIG: Record<
  PageKey,
  { title: string; description: string; viewUrl: string; tabs: TabConfig[] }
> = {
  "gioi-thieu": {
    title: "Trang Giới thiệu",
    description: "Nội dung hiển thị trên trang Giới thiệu của website.",
    viewUrl: "/gioi-thieu",
    tabs: [
    { id: "banner", label: "Banner", save: true },
    { id: "chung", label: "Giới thiệu chung", save: true },
    { id: "su-menh", label: "Sứ mệnh & Trách nhiệm", save: true },
    { id: "lich-su", label: "Lịch sử", save: true },
    { id: "lanh-dao", label: "Ban lãnh đạo", save: true },
    { id: "giai-thuong", label: "Giải thưởng", save: false },
    ],
  },
  "linh-vuc": {
    title: "Trang Lĩnh vực",
    description: "Danh sách các lĩnh vực hoạt động và banner đầu trang.",
    viewUrl: "/linh-vuc",
    tabs: [
    { id: "danh-sach", label: "Danh sách lĩnh vực", save: false },
    { id: "banner", label: "Banner & tiêu đề", save: true },
    ],
  },
  "nang-luc": {
    title: "Trang Năng lực",
    description: "Thiết bị, chỉ số năng lực và chứng nhận của HTX.",
    viewUrl: "/nang-luc",
    tabs: [
    { id: "thiet-bi", label: "Thiết bị", save: false },
    { id: "banner", label: "Banner", save: true },
    { id: "chi-so", label: "Chỉ số tổng quan", save: true },
    { id: "chung-nhan", label: "Chứng nhận", save: true },
    ],
  },
  "tin-tuc": {
    title: "Trang Tin tức",
    description: "Bài viết tin tức và cấu hình đầu trang.",
    viewUrl: "/tin-tuc",
    tabs: [
    { id: "bai-viet", label: "Bài viết", save: false },
    { id: "banner", label: "Banner & danh mục", save: true },
    ],
  },
  "tuyen-dung": {
    title: "Trang Tuyển dụng",
    description: "Vị trí đang tuyển và nội dung trang tuyển dụng.",
    viewUrl: "/tuyen-dung",
    tabs: [
    { id: "vi-tri", label: "Vị trí đang tuyển", save: false },
    { id: "banner", label: "Banner", save: true },
    { id: "phuc-loi", label: "Phúc lợi & bộ lọc", save: true },
    { id: "gui-cv", label: "Khối gửi CV", save: true },
    ],
  },
  "lien-he": {
    title: "Trang Liên hệ",
    description: "Thông tin liên hệ, chi nhánh và bản đồ hiển thị trên website.",
    viewUrl: "/lien-he",
    tabs: [
    { id: "banner", label: "Banner", save: true },
    { id: "nhanh", label: "Liên hệ nhanh", save: true },
    { id: "tru-so", label: "Trụ sở & chi nhánh", save: true },
    { id: "chuyen-biet", label: "Liên hệ chuyên biệt", save: true },
    { id: "ban-do", label: "Bản đồ", save: true },
    ],
  },
}
...
export default function PageContentEditor({
  ...
  const [tab, setTab] = useState<string>(config.tabs[0].id)
  const [content, setContent] = useState<PageEditorContent>(initialContent)
  const [saving, setSaving] = useState(false)
  const [successMessage, setSuccessMessage] = useState('')
  const [errorMessage, setErrorMessage] = useState('')

  const activeTabConfig = config.tabs.find((item) => item.id === tab)
  ...
  const saveContent = async () => {
    ...
    try {
      ...
      setErrorMessage(error?.message || 'Không thể lưu nội dung.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="space-y-5">
      <AdminPageHeader
        ...
        onSave={activeTabConfig?.save ? saveContent : undefined}
        saving={saving}
      />

      <AdminTabs tabs={config.tabs} activeId={tab} onChange={setTab} />
